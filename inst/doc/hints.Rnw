...
class. The S3 class of an object is a character vector, and maybe consist of
...
loaded, since all the relevant methods and functions are in that package.

The {\tt hints} function is compatible with {\tt xtable} in the
